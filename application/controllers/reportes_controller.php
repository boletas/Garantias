...
class Reportes_Controller extends MY_Mantenedor{
    ...
    public function GeneraReportes($fecha,$fecha1,$vence,$periodo,$rut,$tipo,$busqueda, $estado){
        ...
        foreach ($data as $row){
            ...
            $monto = $this->ObtieneMonto($row->idMoneda, ($monto_boleta ? $monto_boleta : $row->monto_boleta));
            $total = $total + $monto;
            ...
            $html .= "<td>".$v['vence']."</td>\n";
            $html .= "<td colspan='2'>".$this->recursos->formateo_moneda($row->codigo,$monto_boleta)."</td>\n";
